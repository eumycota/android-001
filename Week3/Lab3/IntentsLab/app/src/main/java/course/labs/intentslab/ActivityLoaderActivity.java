...
public class ActivityLoaderActivity extends Activity {
    ...
    static public final String USER_TEXT_EXTRA = "course.labs.intentslab.userText";
	...
	private void startExplicitActivation() {
		...
		// TODO1 - Create a new intent to launch the ExplicitlyLoadedActivity class
        Intent intent = new Intent(ActivityLoaderActivity.this, ExplicitlyLoadedActivity.class);
		
		// TODO1 - Start an Activity using that intent and the request code defined above
	...
	private void startImplicitActivation() {
		...
		// TODO1 - Create a base intent for viewing a URL
		// (HINT:  second parameter uses parse() from the Uri class)
        Uri url = Uri.parse(URL);
		Intent browserIntent = new Intent(Intent.ACTION_VIEW, url);

        // TODO1 - Create a chooser intent, for choosing which Activity
		...
		// TODO1 - Start the chooser Activity, using the chooser intent
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        // TODO1 - Process the result only if this method received both a
        // RESULT_OK result code and a recognized request code
        // If so, update the Textview showing the user-entered text.

        if (resultCode == RESULT_OK && requestCode == GET_TEXT_REQUEST_CODE) {
            mUserTextView.setText( data.getStringExtra(USER_TEXT_EXTRA) );
        }
